...
-- 4. Average production duration per product
SELECT Product, round(AVG(Duration_m)) AS avg_duration_minutes
FROM Manufacturing_data
GROUP BY Product
...
-- 1. Most common downtime issues
SELECT 
    'Emergency_stop' AS issue, SUM(Emergency_stop) AS total FROM Downtime
UNION ALL
SELECT 'Machine_failure', SUM(Machine_failure) FROM Downtime
UNION ALL
SELECT 'Labeling_error', SUM(Labeling_error) FROM Downtime
UNION ALL
SELECT 'Inventory_shortage', SUM(Inventory_shortage) FROM Downtime
ORDER BY total DESC;
...
JOIN 
    manufacturing_data m ON d.Batch = m.Batch  -- Join on Batch, not on operator_id
WHERE 
    d.operator_error > 0  -- Filters only operator-related downtime
order by  d.Batch;
